...
export default function HeatExchangerForm() {
  ...
  const chartOptions = (label) => ({
    ...
    scales: {
      x: { title: { display: true, text: 'ṁ𝒸 (kg/s)', font: { size: 16 } } },
      y: { title: { display: true, text: label, font: { size: 16 } } }
    }
  });

  return (
    <div style={{ padding: '2rem' }}>
      <h2 className="text-center mb-4">Double Pipe Heat Exchanger Analysis (Water)</h2>

      <h4>Input Parameters</h4>
      <Table striped bordered responsive>
        <thead className="table-dark text-center">
          <tr><th>Parameter</th>{readings.map((_, idx) => <th key={idx}>Reading {idx + 1}</th>)}</tr>
        </thead>
        <tbody>
          {['mh', 'mc', 'Th_in', 'Th_out', 'Tc_in', 'Tc_out'].map((param, i) => (
            <tr key={i}>
              ...
              {readings.map((r, idx) => (
                <td key={idx}>
                  <input type="number" value={r[param] || ''} onChange={(e) => {
      ...
      {results.length > 0 && (
        <>
          ...
          <Table striped bordered responsive>
            <thead className="table-dark text-center">
              <tr><th>ṁₕ</th><th>ṁ𝒸</th><th>Qₕ</th><th>Q𝒸</th><th>Qavg</th><th>ΔT₁</th><th>ΔT₂</th><th>LMTD</th><th>U</th></tr>
            </thead>
            <tbody>
              {results.map((r, i) => (
                <tr key={i} className="text-center">
                  <td>{r.mh}</td><td>{r.mc}</td><td>{r.Qh.toFixed(2)}</td><td>{r.Qc.toFixed(2)}</td><td>{r.Q.toFixed(2)}</td>
                  <td>{r.deltaT1.toFixed(2)}</td><td>{r.deltaT2.toFixed(2)}</td><td>{r.LMTD.toFixed(2)}</td><td>{r.U.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </Table>
          ...
          <Table striped bordered responsive>
            <thead className="table-dark text-center">
              <tr><th>Reₕ</th><th>Nuₕ</th><th>ΔPₕ</th><th>Re𝒸</th><th>Nu𝒸</th><th>h𝒸</th><th>f𝒸</th><th>ΔP𝒸</th></tr>
            </thead>
            <tbody>
              {results.map((r, i) => (
                <tr key={i} className="text-center">
